...
Add to main.py:
    ...
    app.include_router(share_links.router, prefix="/share-links")
"""
...
class CreateShareLinkRequest(BaseModel):
    asset_id:         str
    permission:       str           # "view_only" | "view_and_download"
    expires_in:       str           # "1d" | "7d" | "30d" | "none"
...
async def create_share_link(
    ...
    log_action(
        current_user["id"], "share_link_created",
        {"asset_id": data.asset_id, "token": link["token"]},
...
async def approve_download_request(
    ...
    # Verify ownership
    link = db.table("share_links").select("id") \
        .eq("id", link_id) \
        .eq("owner_id", current_user["id"]) \
        .execute()
    if not link.data:
    ...
    db.table("share_links").update({
        "download_count": db.table("share_links")
            .select("download_count")
            .eq("id", link_id)
            .execute().data[0]["download_count"] + 1
...
async def get_link_activity(
    ...
    events   = db.table("share_events").select("*") \
        .eq("share_link_id", link_id) \
        .order("created_at", desc=True).execute()
    requests = db.table("download_requests").select("*") \
        .eq("share_link_id", link_id) \
        .order("created_at", desc=True).execute()
...
# ─── Public routes (no auth — called by SharedImagePage in React) ─────────────

@router.get("/public/{token}")
async def get_share_link_public(token: str, request: Request):
    """
    Called by SharedImagePage.jsx to validate the token and get asset info.
    No auth required — this is what recipients hit when they open the link.
    """
    db = get_admin_db()

    result = db.table("share_links") \
        .select("*, vault_images(asset_id, file_name, file_size, resolution, thumbnail_url, owner_name, owner_email, created_at)") \
        .eq("token", token) \
        .execute()
    ...
    if link["status"] == "active":
        ...
        db.table("share_links").update({
            "view_count": (link["view_count"] or 0) + 1
        }).eq("id", link["id"]).execute()

    asset = link.get("vault_images") or {}
...
async def request_download(
    token: str,
    data: DownloadRequestBody,
    request: Request
):
    """Recipient submits a download request. No auth required."""
    db = get_admin_db()

    link_result = db.table("share_links").select("id, status") \
